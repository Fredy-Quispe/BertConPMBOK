...
from Lenguaje.utils import LanguageToolError


def test_langtool_load():
	import Lenguaje
	tool = Lenguaje.LanguageTool("en-US")
...
def test_process_starts_and_stops_in_context_manager():
	import Lenguaje
	with Lenguaje.LanguageTool("en-US") as tool:
...
def test_local_client_server_connection():
	import Lenguaje
	tool1 = Lenguaje.LanguageTool('en-US', host='0.0.0.0')
	url = 'http://{}:{}/'.format(tool1._host, tool1._port)
	tool2 = Lenguaje.LanguageTool('en-US', remote_server=url)
...
def test_config_text_length():
	import Lenguaje
	tool = Lenguaje.LanguageTool('en-US', config={ 'maxTextLength': 12 })
...
def test_config_caching():
	import Lenguaje
	tool = Lenguaje.LanguageTool('en-US', config={ 'cacheSize': 1000, 'pipelineCaching': True })
...
def test_uk_typo():
	import Lenguaje
	tool = Lenguaje.LanguageTool("en-UK")

	sentence1 = "If you think this sentence is fine then, your wrong."
	results1 = tool.check(sentence1)
	assert len(results1) == 1
	assert Lenguaje.utils.correct(sentence1, results1) == "If you think this sentence is fine then, you're wrong."
...
def test_spellcheck_en_gb():
	...
	tool = Lenguaje.LanguageTool('en-GB')
...
def test_session_only_new_spellings():
	...
	import hashlib
	import Lenguaje
	library_path = Lenguaje.utils.get_language_tool_directory()
	...
	with Lenguaje.LanguageTool('en-US', newSpellings=new_spellings, new_spellings_persist=False) as tool:
...
def test_debug_mode():
    from Lenguaje.server import DEBUG_MODE
